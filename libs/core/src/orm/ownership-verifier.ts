...
export class OwnershipVerifier {
    public static verifyOrFail(user: User | null, model: ObjectLiteral, key = 'userId'): void {
        if (!OwnershipVerifier.verify(user, model, key)) {
            throw new ForbiddenException();
        }
    }

    public static verify(user: User | null, model: ObjectLiteral, key = 'userId'): boolean {
        return user && user.id === model[key];
    }
}
